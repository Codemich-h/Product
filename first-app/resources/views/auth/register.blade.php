<x-guest-layout>
    <form method="POST" action="{{ route('register') }}" class="space-y-6">
        ...
        @endif
        <div class="mb-12">
            <h1 class="text-slate-900 text-3xl font-semibold ml-32">Sign Up</h1>
            <p class="text-slate-600 text-[15px] mt-6 leading-relaxed ml-20">Sign up to your new account.</p>
        </div>
        <div>
            <label class="text-slate-900 text-sm font-medium mb-2 block">Name</label>
            <div class="relative flex items-center">
                <input for="name" :value="__('Name')" name="name" type="text" required
        ...
        <div>
            <label for="password_confirmation" class="text-slate-900 text-sm font-medium mb-2 block">Confirmed
                Password</label>
            <div x-data="{ show: true }" class="relative flex items-center">
                <input id="password_confirmation" name="password_confirmation" :type="show ? 'password' : 'text'"
                    required for="password_confirmation"
                    class="w-full text-sm text-slate-900 border border-slate-300 pl-4 pr-10 py-3 rounded-lg outline-blue-600"
                    placeholder="Enter password" />
                <button type="button" class="flex absolute inset-y-0 right-0 items-center pr-3" @click="show = !show"
                    :class="{ 'hidden': !show, 'block': show }">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="!mt-12">
                ...
                <p class="text-sm !mt-6 text-center text-slate-600">Already registered?<a href="{{ route('login') }}"
                        class="text-blue-600 font-medium hover:underline ml-1 whitespace-nowrap">Click here</a></p>
            </div>
        </div>
    </form>
</x-guest-layout>
